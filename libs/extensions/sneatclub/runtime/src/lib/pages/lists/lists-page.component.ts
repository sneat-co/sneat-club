...
export class ListsPageComponent extends SpaceBaseComponent {
  private readonly params = inject(SneatclubComponentBaseParams);
  ...
  constructor() {
    ...
    this.sneatclubAppStateService.changed.subscribe((appState) => {
  ...
  public clickGroup(group: IListGroup): void {
    ...
    this.sneatclubAppStateService.setGroupCollapsed(
      group.title,
    ...
    try {
      ...
      if (this.space) {
        if (this.reordered) {
          ...
          const sneatclubDbo = this.space.dbo as unknown as
            | ISneatclubSpaceDbo
            | undefined;
          this.updateListsFromSpace(sneatclubDbo?.listGroups); // persisted lists
        }
      } else {
        this.listGroups = [];
      }
