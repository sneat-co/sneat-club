...
import { SneatclubCoreServicesModule } from '../../services';
...
import { SneatclubComponentBaseParams } from '../../sneatclub-component-base-params';
import { ISneatclubAppStateService } from '../../services';
...
@Component({
  selector: 'sneatclub-list',
  templateUrl: './list-page.component.html',
  imports: [
    SneatclubCoreServicesModule,
    NgOptimizedImage,
    ListItemComponent,
...
export class ListPageComponent extends BaseListPage implements AfterViewInit {
  ...
  private readonly sneatclubAppStateService = inject(ISneatclubAppStateService);
  ...
  constructor() {
    const params = inject(SneatclubComponentBaseParams);

    super(params);
    const sneatclubAppStateService = this.sneatclubAppStateService;
    this.preloader.markAsPreloaded('list');
    if (location.pathname.includes('/lists')) {
      // TODO: document why & how it is possible
      return;
    }
    sneatclubAppStateService.changed.subscribe((appState) => {
  ...
  public clickShowWatchedMovies(): void {
    this.sneatclubAppStateService.setShowWatched(this.isHideWatched());
  }
  ...
  // 	if (this.appService.appId === 'sneatclub') {
